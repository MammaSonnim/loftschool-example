...
function findAllPSiblings(where) {
    var siblings = where.children;
    var allPSiblings = [];

    for (var i = 0; i < siblings.length; i++) {
        if (siblings[i].nextElementSibling && siblings[i].nextElementSibling.tagName === 'P') {
            allPSiblings.push(siblings[i]);
        }
    }
...
function findError(where) {
    ...
    for (var i = 0; i < siblings.length; i++) {
        textNodes.push(siblings[i].innerText);
    }
